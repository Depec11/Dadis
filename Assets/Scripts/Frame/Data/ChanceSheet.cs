...
namespace Frame.Data {
    ...
    public class ChanceSheet<T> : ScriptableObject {
        ...
        public ChanceSheet() {
            if (pairList == null) {
                pairList = new List<Pair<float, T>>();
            }
        }
        /// <summary>
        /// 重载转字符串函数
        ...
        public T SearchByChance(float chance) {
            for (int i = 0; i < pairList.Count; i++) {
                if (chance >= pairList[i].first) {
                    chance -= pairList[i].first;
                }
                else {
                    return pairList[i].second;
                }
            }
            return default;
        }
        ...
        public Pair<float, T> SearchByValue(T value) {
            for (int i = 0; i < pairList.Count; i++) {
                if (EqualityComparer<T>.Default.Equals(pairList[i].second, value)) {
                    return pairList[i];
                }
            }
        ...
        public ChanceSheet<T> Copy() {
            ...
            for (int i = 0; i < pairList.Count; i++) {
                tempList[i] = new Pair<float, T>(pairList[i].first, pairList[i].second);
            }
